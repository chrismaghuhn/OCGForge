...
| Decision context kind | `PublicEnvironmentObservation.decision_context.kind` | DIRECT | Copy the optional canonical public request-family token. | Context is already projected safe. | Canonical token bytes are stable. | Malformed token is `INVALID`; absent value is `UNSUPPORTED` where required. | BattleCommand shape checks |
...
| Visible entity controller | Exact entity in same-perspective `PublicSafeStateView.entities` | DIRECT | Resolve an exact `observation_locator`; copy public controller when present. | Same-perspective public state only. | Exactly one locator match; no map-order dependency. | Missing/duplicate locator is `INVALID`; absent controller is `UNSUPPORTED` unless structurally contradictory; redacted identity-dependent join is `UNSUPPORTED`. | Candidate context |
| Visible entity zone | Exact public entity zone | DIRECT | Resolve exact locator and copy a concrete public zone; `SemanticZone::Unknown` maps to no optional zone value. | Zone is part of the safe projection without inventing a concrete location. | Scalar copy after exact join. | Missing/duplicate join is `INVALID`; `SemanticZone::Unknown` makes zone-dependent derivation `UNSUPPORTED`; invalid enum is `INVALID`. | Source/target context |
| Visible entity position | Exact public entity position | DIRECT | Resolve exact locator and copy a concrete public position; `Position::Unknown` maps to no optional position value. | Position is public only for the projected entity and only when concrete. | Scalar copy. | Missing/duplicate join is `INVALID`; `Position::Unknown` makes position-dependent derivation `UNSUPPORTED`; invalid enum is `INVALID`. | Battle candidate facts |
