...
`ocgforge.public_environment_observation.v1`, with nested canonical bytes owned
by `ocgforge.public_safe_state.v1`. The projection is produced from the
perspective-safe fields of `ygo.player_observation.v1`, but it is not that
record's canonical serialization and its safe-state bytes are generated by the
public projection helper rather than supplied by a caller. In particular,
attached internal decision and continuation IDs and the v1 observation hash
