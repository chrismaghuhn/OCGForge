...
| **G28 Maximum-candidate witness** | Turn aggregate M4 candidate maximum into reproducible evidence. | Accepted deterministic workload emits one canonical tie-broken witness whose candidate count equals the aggregate maximum and whose episode/frame/request/continuation/raw-message/domain/observation identities reproduce exactly under replay. No candidate cap and witness is not policy input. | MAJOR | `max_candidate_witness.json` |
