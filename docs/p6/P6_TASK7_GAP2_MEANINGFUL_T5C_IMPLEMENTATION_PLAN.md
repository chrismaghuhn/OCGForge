...
### 5.1 Validated checkpoint proof

The accepted checkpoint loader remains owner of manifest schema, canonical
manifest bytes, export content, and checkpoint identity. Before the meaningful
factory runs, it must provide a validated typed view named:

\`\`\`text
MeaningfulCheckpointBindingV1
\`\`\`

It carries or exposes:
...
The proof is a validated view, not a second manifest codec or digest. Dataset
and split remain transitive checkpoint provenance and remain absent from
gameplay job optionals.
...
\`\`\`cpp
struct MeaningfulCheckpointBindingV1 final {
    // validated checkpoint/manifest/config/vocabulary/dataset/export facts
};
...
These names and separation are part of the design. Private execution helpers
may be shared, but the public smoke validator may not become permissive. The
existing \`make_implementation_acceptance_context(...)\` and
...
| Checkpoint | malformed/mixed/mismatched checkpoint, artifact, provenance, export, or Task7 config rejected |
