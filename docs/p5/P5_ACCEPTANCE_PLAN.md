...
| --- | --- | --- | --- |
| P5-G00 | historical Task-1 contract IDs, owner, and docs-only scope are frozen | exact Task-1 base/head comparison plus ADR/contract/index review | NOT_RUN |
...
### P5-G00 — historical Task-1 freeze scope

This gate checks the completed historical Task-1 freeze as a fixed checkpoint,
not the scope of a later implementation head. Verify all of the following:
...
of the documents as a P5 implementation PASS or impose the four-file condition
on a later implementation head.
...
Request kind/player may be read from the observation's already-public decision
context or checked for equality when an associated public frame provides them;
no request-only state is a separate model input. In particular,
EnvironmentContinuationView is not supplied to the model representation.
...
The test may exercise atomic and candidate-vector continuation operations such
as pick, amount, finish, and cancel, but it must not read or encode an
EnvironmentContinuationView as model input.
...
### P5-G17 — final implementation clean-checkout and authorized scope

Create a fresh clean checkout from the eventual P5 implementation head and
verify:

- the checked-out head equals the reported final implementation head;
- the original base and the separate historical P5-G00 Task-1 freeze pair are
  recorded;
- the worktree is clean before and after acceptance;
- the final diff is limited to the authorized P5 implementation scope. That
  scope may include ygo::model production code, its tests, generated evidence,
  and the explicitly authorized P5 documentation. It does not include gameplay,
  Teacher, trusted trajectory semantics, rules, decks, CMake, or dependencies
  unless a later task explicitly authorizes that change;
- the four Task-1 documents contain no unresolved conflict markers or
  placeholders, but they are not the only files allowed at the final head;
- all required P5 evidence is generated by commands, not hand-edited; and
- applicable Phase-3/4 regression results are independently reproducible.

P5-G17 is a final implementation-head gate. It does not re-use the historical
four-file Task-1 diff condition. A clean checkout is necessary but does not turn
unrun semantic gates into PASS.
