...
`ocgforge.trusted_trajectory.v1`, `ocgforge.policy_provenance.v1`, their
supporting identity/provenance domains, and
`ocgforge.restricted_replay_evidence.v1`. The complete normative domain list
is in ADR-0005. This is not runtime acceptance evidence and does **not** claim
`TRUSTED TRAJECTORY V1 FINAL PASS`.
...
| P3A-G02 Temporal convention complete | Make one record mean one accepted semantic action. | `F_t -- a_t -> F_(t+1)` or closure is exact; `NEXT_FRAME` targets exactly either the next `DecisionRecord.frame` or the final `INTERRUPTED` pending-unacted frame; successor observations belong to the next frame; first frame, zero-decision terminal, interruption, failure, and administrative cancellation are all specified. | BLOCKER | trajectory core | Golden sequence tests for atomic, zero-decision terminal, direct interruption, administrative pending-frame interruption, and failure boundaries. |
...
| P3A-G07 Exact policy-producer attribution | Identify the concrete producer for every accepted decision. | Policy artifact, participant assignment, seat/deck/policy roles, assignment epochs, artifact adapters, sampling contract, concrete RNG initialization, and per-record assignment resolution are fully encoded and fail closed. | BLOCKER | policy provenance | Fixture corpus for RandomLegal, deterministic heuristic, checkpoint, search-assisted, and imported-demonstration provenance; malformed-resolution and initialization-identity negatives. |
| P3A-G08 Policy RNG provenance separation | Attribute stochastic choice without importing engine RNG semantics. | Policy RNG contract, concrete initialization identity, stream identity, and pre/post `NONE`/cursor/state modes are encoded; `CURSOR` is valid only with unique initialized-stream semantics; environment root seed and execution/hardware entropy are excluded from RNG and gameplay identity. | BLOCKER | policy provenance | Deterministic RNG-provenance vectors, distinct-seat and distinct-initialization stream tests, cursor-ambiguity/state-mode negatives, malformed-state tests, and proof that replay does not consume policy RNG. |
| P3A-G09 Terminal/interrupted/failed closure semantics | Preserve outcome truth without inventing value labels. | Only true terminal carries winner/win reason/views; interrupted carries no game outcome or reward and requires exact restricted reason/run-control/count/engine-step evidence for admission replay; failed has no fabricated outcome and is not learner eligible; zero-decision terminal is explicit. | BLOCKER | trajectory closure | Closure golden tests for terminal, zero-decision terminal, budget interruption, administrative cancellation, failure quarantine, and missing/mismatched restricted interruption evidence. |
...
| P3A-G13 Recurrent/POMDP perspective isolation | Prevent mixed-seat recurrent state or private observation joins. | `AgentDecisionStream(episode, participant_assignment)` contains only one assignment’s public records in global order, only its own terminal view, and a pending frame only when it is the acting assignment; same checkpoint on two seats remains separate; burn-in/TBPTT/state are derived. | BLOCKER | future model adapter | Paired-world derived-stream tests showing no cross-player concatenation, opponent terminal view, global identity, reset bytes, or extra canonical state fields. |
| P3A-G14 Canonical-versus-derived boundary | Keep the source data algorithm-neutral and regenerable. | The contract lists canonical values and the required derived values, including tensors, indexes, rewards, recurrence, returns, priorities, exports, splits, and normalization; raw global envelopes are never learner input and derived data cannot change core identities. | BLOCKER | trajectory core | Projection reproducibility tests from canonical envelope plus explicit derivation configuration; identity invariance and participant-projection deny-list tests. |
| P3A-G15 Canonical codec/hash field order complete | Leave Phase 3B no semantic codec guesswork. | All new IDs and restricted interruption evidence have exact domain/schema strings, lexical forms where applicable, field order, primitive codecs, enum codes, exclusions, and mutation rule; frame/request/candidate/continuation/closure/manifest codecs are specified. | BLOCKER | identity and codec contracts | Cross-process golden codecs, SHA-256 vectors, parser rejection tests, initialization/replay-evidence vectors, and independent implementation comparison. |
...
| P3A-G17 Phase-3B persistence/admission handoff complete | Constrain persistence without prematurely designing it. | The contract defers physical artifacts but fixes the handoff from canonical episode plus required interrupted-case restricted replay evidence through structural/privacy validation and semantic replay to future receipt/manifest; it names all deferred storage concerns. | MAJOR | Phase 3B admission | Candidate-shard structural validation, semantic replay including interruption evidence, cryptographic candidate/evidence receipt binding, and dataset-manifest integration tests. |
...
- a hidden-information oracle through direct values, hashes, global identities,
  terminal views, or cross-participant joins;
- accidental internal-ID, token, response, audit, or restricted replay-evidence
  persistence into learner data;
- candidate-index authority or domain reshaping;
- ambiguous reward timing, duplicated successor observation, or a
  pending-unacted frame misrepresented as an accepted action;
- a fake terminal action or ambiguous zero-decision closure;
- episode-global behavior-policy attribution or ambiguous policy-RNG
  initialization/cursor semantics;
- mixed-player recurrent streams;
- an interrupted replay claim without exact restricted run-control evidence and
...
+ RestrictedReplayEvidence when closure == INTERRUPTED
  ...
  -> immutable AdmissionReceipt binding the verified candidate and evidence
  -> DatasetManifest
```
...
privacy boundary, identity hierarchy, and required interrupted-case evidence
binding. It does not authorize Teacher, model, training, tensorization, or
framework work.
