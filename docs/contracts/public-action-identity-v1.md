...
This contract owns three separate policy-facing identity domains:
...
| 3 | source reference | optional `{kind:u8, observation_locator:string}` |
| 4 | target reference | optional `{kind:u8, observation_locator:string}` |
| 5 | phase | optional `u32` |
| 6 | position | optional `u8` |
| 7 | source index | optional `u32` |
| 8 | amount | optional signed `i32`, encoded as its two's-complement `u32` bits |
| 9 | continuation operation | canonical lower-case token string, empty when absent |
...
4. construct exactly one public action key from the audited safe descriptor;
5. preserve the full candidate count and membership; and
6. reject the whole frame with a structured privacy/projection failure if any
...
| 6 | public candidate-domain digest | lowercase SHA-256 string |

The public decision ID contains only public decision position, acting-player
coupling, request family, and the public domain identity. It deliberately
does not contain or commit to:

- protocol `DecisionRequest.decision_id`;
- `engine_step_index`;
- `PlayerObservation.observation_hash` when its canonical v1 context may
  carry internal decision/continuation metadata;
...
The full `PlayerObservation` remains the policy state in the public frame.
Excluding its internal-bearing hash from this separate public identity is
intentional: it prevents a public semantic ID from becoming a transitive
carrier for internal protocol or continuation identity.
...
2. perform the same perspective-safe projection in the same order;
3. recompute the public candidate-domain digest and public semantic decision
   ID;
4. resolve the recorded public key against the regenerated frame-local
   binding table; and
5. submit the resolved existing internal semantic key through `EpisodeDriver`.
...
The internal semantic keys may differ. The public key resolves to the one
current internal candidate separately in each world; it must not be used to
claim that the hidden physical cards are the same.
