...
PublicEnvironmentObservation public_observation(const std::uint32_t self_life_points = 8000,
...
TeacherStateDeltaV1 replacement_delta(const EpisodeLocalStrategyStateV1& state,
    ...
    value.achieved_goal_ids = {"goal.achieved"};
...
void test_pure_proposal_and_accepted_commit() {
    ...
    require(ygo::teacher::commit_teacher_state_delta(
                committed, ranking_result(*proposed, selected_key),
...
void test_observation_dominance_and_repeated_reconciliation() {
    ...
    require(ygo::teacher::commit_teacher_state_delta(
                dominated, ranking_result(*proposed, selected_key),
    ...
    auto right = first;
    require(ygo::teacher::commit_teacher_state_delta(
                left, ranking_result(*proposed, selected_key),
                accepted_transition(2, selected_key),
                public_observation()),
            "first identical reconciliation failed");
    require(ygo::teacher::commit_teacher_state_delta(
                right, ranking_result(*proposed, selected_key),
                accepted_transition(2, selected_key),
                public_observation()),
...
void test_commit_mismatches_are_atomic() {
    ...
    const auto attempt = [&](const TeacherStateDeltaV1& delta,
        ...
        require(!ygo::teacher::commit_teacher_state_delta(
                    state, ranking_result(delta, selected), transition, observation),
