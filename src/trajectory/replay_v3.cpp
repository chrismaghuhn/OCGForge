...
bool snapshot_from_frame(const environment::DecisionFrame& frame,
                         PublicFrameSnapshotV3& output,
                         std::string& error) {
    if (frame.contract_id != environment::kEpisodicEnvironmentV4ContractId ||
        !frame.submission_token.valid()) {
        error = "V3 replay received a non-V3 frame or invalid submission token";
        return false;
    }
    ...
    try {
        (void)canonical_public_frame_snapshot_bytes_v3(output);
        return true;
...
bool equal_frame(const PublicFrameSnapshotV3& expected,
                 const environment::DecisionFrame& actual,
                 std::string& error) {
    PublicFrameSnapshotV3 actual_snapshot;
    if (!snapshot_from_frame(actual, actual_snapshot, error)) {
        return false;
    }
    try {
        if (canonical_public_frame_snapshot_bytes_v3(expected) !=
            canonical_public_frame_snapshot_bytes_v3(actual_snapshot)) {
            error = "regenerated V3 public frame differs from the V3 recorded frame";
            return false;
        }
        return true;
    } catch (const std::exception& exception) {
...
bool equal_observation(const environment::PublicEnvironmentObservation& expected,
    ...
    try {
        if (environment::canonical_public_environment_observation_bytes(expected) !=
            ...
            error = "regenerated V3 terminal observation differs from the V3 recorded view";
            return false;
        }
        return true;
    } catch (const std::exception& exception) {
...
bool resolve_identity_inputs(const EpisodeEnvelopeV3& envelope,
    ...
    if (!decoded_config || !is_current_certified_environment_v4(*decoded_config.value)) {
        error = "V3 replay environment identity is not the current V3 environment";
        return false;
    }
    ...
    if (environment::canonical_environment_identity_bytes(config) !=
        ...
        error = "V3 replay environment identity bytes or digest differ from the manifest";
        return false;
    }
    ...
    if (!decoded_spec ||
        ...
        error = "V3 replay episode identity bytes or digest differ from the manifest";
        return false;
    }
...
std::optional<environment::RunControl> replay_control(
    ...
    std::string& error) {
    if (std::holds_alternative<TerminalClosureV3>(envelope.closure)) {
        if (evidence.has_value()) {
            error = "V3 terminal replay received interruption evidence";
            return std::nullopt;
        }
        if (!options.terminal_run_control.has_value() ||
            !valid_control(*options.terminal_run_control)) {
            error = "V3 terminal replay requires an explicit valid run control";
            return std::nullopt;
        }
    ...
    if (!evidence.has_value() ||
        ...
        error = "V3 interrupted replay lacks exact restricted evidence";
        return std::nullopt;
    }
    ...
    if (!valid_control(control)) {
        error = "V3 replay cancellation source is invalid";
        return std::nullopt;
    }
    return control;
}
...
bool expected_transition_class(const environment::EnvironmentActionCandidate& candidate,
    ...
    if (!request.continuation.has_value()) {
        if (!candidate.submits_engine_response || !core_response_submitted) {
            error = "V3 atomic transition did not submit exactly one response";
            return false;
        }
    ...
    if (candidate.submits_engine_response) {
        if (!core_response_submitted) {
            error = "V3 final continuation did not submit a response";
            return false;
        }
    ...
    if (core_response_submitted) {
        error = "V3 intermediate continuation submitted a response";
        return false;
    }
...
bool compare_terminal(const EpisodeEnvelopeV3& envelope,
                      environment::EpisodicEnvironment& environment,
                      const environment::EpisodeTerminal& actual,
                      std::string& error) {
    const auto* expected = std::get_if<TerminalClosureV3>(&envelope.closure);
    if (expected == nullptr || actual.contract_id != environment::kEpisodicEnvironmentV4ContractId ||
        ...
        error = "V3 terminal boundary differs from the V3 recorded closure";
        return false;
    }
    ...
    if (environment::public_observation_digest(*player_zero) !=
        ...
        error = "V3 terminal observation digest differs from the V3 closure";
        return false;
    }
    return true;
}

bool compare_interrupted(const EpisodeEnvelopeV3& envelope,
                         const RestrictedReplayEvidenceV3& evidence,
                         const environment::EpisodeInterrupted& actual,
                         const bool administrative_pending,
                         std::uint64_t& final_engine_step,
                         std::string& error) {
    const auto* expected = std::get_if<InterruptedClosureV3>(&envelope.closure);
    if (expected == nullptr || actual.contract_id != environment::kEpisodicEnvironmentV4ContractId ||
        actual.episode_semantic_id != envelope.manifest.episode_semantic_id ||
        ...
        error = "V3 interruption boundary differs from V3 restricted evidence";
        return false;
    }
    if (administrative_pending) {
        if (!expected->pending_unacted_frame.has_value() ||
            ...
            error = "V3 administrative interruption does not identify its pending frame";
            return false;
        }
    } else {
        ...
        if (actual.last_decision_index != expected_last_index ||
            ...
            error = "V3 interruption prefix differs from the recorded action prefix";
            return false;
        }
    }
...
bool same_public_frame_as_snapshot(
    const PublicFrameSnapshotV3& expected,
...
ReplayResult replay_episode_v3(
    ...
    try {
        ...
        if (std::holds_alternative<FailedClosureV3>(envelope.closure)) {
            fail(result, "failed V3 envelope has no normal replay admission");
            return result;
        }
        if (evidence.has_value()) {
        ...
        for (std::size_t index = 0; index < envelope.records.size(); ++index) {
            ...
            if (current_frame == nullptr) {
                fail(result, "V3 replay reached a non-frame boundary before all records");
                return result;
            }
            ...
            if (selected_count != 1 || selected == nullptr ||
                !environment::is_public_action_key_v3(record.selected_public_action_key)) {
                fail(result, "V3 selected action key is not unique in the regenerated V3 domain");
                return result;
            }
            if (record.transition_class != declared_transition_class(*selected, current_frame->request)) {
                fail(result, "V3 transition class differs from regenerated V3 semantics");
                return result;
            }

            environment::ActionSelection selection;
            selection.contract_id = std::string(environment::kEpisodicEnvironmentV4ContractId);
            ...
            if (step_accepted == nullptr ||
                ...
                fail(result, "V3 rejected or changed the recorded V3 public action");
                return result;
            }
            ...
            if (record.successor.kind == SuccessorKind::NextFrame) {
                ...
                if (next_frame == nullptr || next_frame_target == nullptr) {
                    fail(result, "V3 successor is not a regenerated V3 frame");
                    return result;
                }
                if (next_frame_target->kind == NextFrameTargetKind::InterruptionPendingUnactedFrame) {
                    ...
                    const auto* interrupted =
                        std::get_if<InterruptedClosureV3>(&envelope.closure);
                    if (interrupted == nullptr || !interrupted->pending_unacted_frame.has_value() ||
                        ...
                        fail(result, error.empty() ? "pending V3 frame differs" : std::move(error));
                        return result;
                    }
                    boundary = *next_frame;
                ...
                if (next_frame_target->next_decision_index != next_frame->decision_index ||
                    ...
                    fail(result, "V3 successor frame differs from the recorded sequence");
                    return result;
                }
                boundary = *next_frame;
                continue;
            ...
            if (index + 1 != envelope.records.size()) {
                fail(result, "non-final V3 record has a closure successor");
                return result;
            }
            if (record.successor.kind == SuccessorKind::Terminal) {
                if (!std::holds_alternative<environment::EpisodeTerminal>(step_accepted->next) ||
                    !std::holds_alternative<TerminalClosureV3>(envelope.closure)) {
                    fail(result, "V3 terminal successor does not match V3");
                    return result;
                }
                ...
                if (!std::holds_alternative<environment::EpisodeInterrupted>(step_accepted->next) ||
                    ...
                    std::get<InterruptedClosureV3>(envelope.closure).pending_unacted_frame.has_value()) {
                    fail(result, "V3 interrupted successor does not match V3");
                    return result;
                }
            } else {
                fail(result, "V3 failed successor cannot be admitted");
                return result;
            }
        ...
        if (pending_administrative_interrupt) {
            ...
            const auto* closure = std::get_if<InterruptedClosureV3>(&envelope.closure);
            ...
            if (accepted_interrupt == nullptr || !evidence.has_value() ||
                ...
                fail(result, error.empty() ? "V3 administrative interruption differs" : std::move(error));
                return result;
            }
        ...
        if (envelope.records.empty() &&
            ...
            const auto& expected_pending =
                *std::get<InterruptedClosureV3>(envelope.closure).pending_unacted_frame;
            if (pending_frame == nullptr || !equal_frame(expected_pending, *pending_frame, error)) {
                fail(result, error.empty() ? "initial pending V3 frame differs" : std::move(error));
                return result;
            }
            const auto interrupted = environment->interrupt(environment::InterruptRequest{
                std::string(environment::kEpisodicEnvironmentV4ContractId),
            ...
            if (accepted_interrupt == nullptr || !evidence.has_value() ||
                !compare_interrupted(envelope, *evidence, accepted_interrupt->interruption, true,
                                     result.final_engine_step_index, error)) {
                fail(result, error.empty() ? "initial V3 interruption differs" : std::move(error));
                return result;
            }
            result.accepted = true;
            return result;
            ...
            if (const auto* interrupted = std::get_if<environment::EpisodeInterrupted>(&boundary)) {
                if (!evidence.has_value() ||
                    !compare_interrupted(envelope, *evidence, *interrupted, false,
                                         result.final_engine_step_index, error)) {
                    fail(result, error.empty() ? "initial V3 interruption lacks evidence"
                                               : std::move(error));
                    return result;
                }
                result.accepted = true;
        ...
        if (const auto* interrupted = std::get_if<environment::EpisodeInterrupted>(&boundary)) {
            if (!evidence.has_value() ||
                !compare_interrupted(envelope, *evidence, *interrupted, false,
                                     result.final_engine_step_index, error)) {
                fail(result, error.empty() ? "V3 interruption lacks evidence" : std::move(error));
                return result;
            }
            result.accepted = true;
            return result;
        }
        fail(result, "V3 replay ended in an unsupported failure boundary");
        return result;
    } catch (const std::exception& exception) {
        fail(result, exception.what());
        return result;
    } catch (...) {
        fail(result, "V3 replay threw");
        return result;
    }
}
