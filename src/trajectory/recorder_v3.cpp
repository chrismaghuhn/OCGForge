...
TrajectoryRecorderV3::TrajectoryRecorderV3(
    ...
    if (!is_current_certified_environment_v4(config_) ||
        config_.contract_id != environment::kEpisodicEnvironmentV4ContractId ||
        spec_.contract_id != environment::kEpisodicEnvironmentV4ContractId) {
        throw std::invalid_argument("V3 trajectory recorder requires current V3 environment");
    }
    ...
    if (!config_.environment_semantic_id.empty() &&
        ...
        throw std::invalid_argument("V3 recorder environment identity mismatch");
    }
    ...
    if (!resolver_.validate(manifest_.policy_provenance, config_, spec_, &error)) {
        throw std::invalid_argument("V3 recorder provenance is invalid: " + error);
    }
    (void)canonical_episode_manifest_bytes_v3(manifest_);
}

bool TrajectoryRecorderV3::set_error(std::string* error, std::string message) const {
    if (error != nullptr) {
...
bool TrajectoryRecorderV3::capture_frame(
    ...
    try {
        if (frame.contract_id != environment::kEpisodicEnvironmentV4ContractId ||
            ...
            return set_error(error, "V3 frame does not match V3 recorder boundary");
        }
        PublicFrameSnapshotV3 snapshot;
        ...
        (void)canonical_public_frame_snapshot_bytes_v3(snapshot);
...
bool TrajectoryRecorderV3::capture_terminal(
    const environment::EpisodeTerminal& terminal,
    const TerminalViews& views,
    std::string* error) {
    try {
        if (terminal.contract_id != environment::kEpisodicEnvironmentV4ContractId ||
            ...
            return set_error(error, "V3 terminal does not match V3 recorder boundary");
        }
        ...
        (void)canonical_episode_closure_bytes_v3(closure);
        closure_ = std::move(closure);
        return true;
    } catch (const std::exception& exception) {
        return set_error(error, exception.what());
...
bool TrajectoryRecorderV3::capture_interruption(
    const environment::EpisodeInterrupted& interruption,
    std::string* error) {
    if (interruption.contract_id != environment::kEpisodicEnvironmentV4ContractId ||
        interruption.episode_semantic_id != manifest_.episode_semantic_id ||
        interruption.semantic_action_count != records_.size()) {
        return set_error(error, "V3 interruption does not match V3 recorder boundary");
    }
    ...
    InterruptedClosureV3 closure;
    closure.record_count = records_.size();
    closure_ = std::move(closure);
    return true;
}

bool TrajectoryRecorderV3::capture_failure(
    ...
    const auto expected_last_id = current_frame_.has_value()
                                      ...
                                      : (records_.empty()
                                             ...
                                             : std::optional<std::string>{
                                                   records_.back().frame.public_semantic_decision_id});
    if (failure.contract_id != environment::kEpisodicEnvironmentV4ContractId ||
        ...
        return set_error(error, "V3 failure does not match V3 recorder boundary");
    }
    FailedClosureV3 closure;
...
bool TrajectoryRecorderV3::on_reset_accepted(
    ...
    if (lifecycle_ != RecorderLifecycle::Empty || !records_.empty() ||
        closure_.has_value()) {
        return set_error(error, "V3 recorder reset is not at EMPTY");
    }
...
bool TrajectoryRecorderV3::on_step_accepted(
    const environment::StepAccepted& accepted,
    const PolicyRngDecisionProvenance& attribution,
    const std::optional<TerminalViews>& terminal_views,
    std::string* error) {
    if (lifecycle_ != RecorderLifecycle::AwaitingAction || !current_frame_.has_value()) {
        return set_error(error, "V3 recorder accepted action outside an awaiting frame");
    }
    ...
    if (!current.request.continuation.has_value()) {
        if (!selected->submits_engine_response ||
            ...
            return fail_closed(environment::FailureCode::ResponseInconsistency,
                               ...
                               "V3 atomic transition classification is inconsistent");
        }
        ...
        if (!accepted.transition.core_response_submitted) {
            return fail_closed(environment::FailureCode::ResponseInconsistency,
                               environment::FailureStage::Advance, true, error,
                               "V3 final continuation response was not submitted");
        }
        ...
        if (accepted.transition.core_response_submitted) {
            return fail_closed(environment::FailureCode::ResponseInconsistency,
                               environment::FailureStage::Advance, true, error,
                               "V3 intermediate continuation submitted a response");
        }
    ...
    if (!expected_assignment.has_value() ||
        ...
        return fail_closed(environment::FailureCode::PublicFrameInvariant,
                           environment::FailureStage::Action, true, error,
                           "V3 policy attribution does not resolve to acting assignment");
    }
    ...
    std::optional<PublicFrameSnapshotV3> next_frame;
    std::optional<EpisodeClosureV3> next_closure;
    if (const auto* frame = std::get_if<environment::DecisionFrame>(&accepted.next)) {
        ...
        if (!terminal_views.has_value() ||
            terminal->contract_id != environment::kEpisodicEnvironmentV4ContractId ||
            terminal->episode_semantic_id != manifest_.episode_semantic_id ||
        ...
        next_closure = std::move(closure);
                   ...
                   std::get_if<environment::EpisodeInterrupted>(&accepted.next)) {
        if (interrupted->contract_id != environment::kEpisodicEnvironmentV4ContractId ||
        ...
        const auto* failure = std::get_if<environment::EpisodeFailure>(&accepted.next);
        if (failure == nullptr ||
            failure->contract_id != environment::kEpisodicEnvironmentV4ContractId ||
            (failure->episode_semantic_id.has_value() &&
             *failure->episode_semantic_id != manifest_.episode_semantic_id) ||
    ...
    try {
        (void)canonical_collection_decision_record_bytes_v3(record);
        if (next_closure.has_value()) {
            (void)canonical_episode_closure_bytes_v3(*next_closure);
        }
    } catch (const std::exception& exception) {
        return fail_closed(environment::FailureCode::PublicFrameInvariant,
                           environment::FailureStage::Projection, true, error,
...
bool TrajectoryRecorderV3::on_step_rejected(
    const environment::StepRejected& rejected,
    const bool policy_origin,
    std::string* error) {
    if (lifecycle_ != RecorderLifecycle::AwaitingAction || !current_frame_.has_value()) {
        return set_error(error, "V3 recorder rejection is outside an awaiting frame");
    }
    if (rejected.contract_id != environment::kEpisodicEnvironmentV4ContractId ||
...
bool TrajectoryRecorderV3::on_interrupt_accepted(
    ...
    if (lifecycle_ != RecorderLifecycle::AwaitingAction ||
        ...
        return set_error(error, "V3 interruption lacks its pending frame");
    }
    ...
    std::optional<PublicFrameSnapshotV3> pending;
    if (!capture_frame(*pending_frame, pending, records_.size(), error) ||
        !pending.has_value() ||
        canonical_public_frame_snapshot_bytes_v3(*pending) !=
            canonical_public_frame_snapshot_bytes_v3(*current_frame_)) {
        return fail_closed(environment::FailureCode::PublicFrameInvariant,
                           environment::FailureStage::Interruption, true, error,
    ...
    try {
        (void)canonical_episode_closure_bytes_v3(closure);
        if (!records_.empty()) {
            (void)canonical_collection_decision_record_bytes_v3(records_.back());
        }
    } catch (const std::exception& exception) {
        return fail_closed(environment::FailureCode::PublicFrameInvariant,
                           environment::FailureStage::Interruption, true, error,
...
bool TrajectoryRecorderV3::on_failure(
    const environment::EpisodeFailure& failure,
    std::string* error) {
    if (lifecycle_ != RecorderLifecycle::AwaitingAction || !current_frame_.has_value()) {
        return set_error(error, "V3 recorder failure is outside an awaiting frame");
    }
...
std::optional<EpisodeEnvelopeV3> TrajectoryRecorderV3::seal(std::string* error) const {
    if (!closure_.has_value()) {
        if (error != nullptr) {
            *error = "V3 recorder has no closure";
        }
        return std::nullopt;
    }
    try {
        ...
        if (error != nullptr) {
            *error = "sealing V3 trajectory recorder threw";
        }
        return std::nullopt;
    }
}
