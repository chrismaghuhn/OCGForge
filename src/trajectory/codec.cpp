...
void validate_continuation(const environment::EnvironmentContinuationView& value) {
    ...
    // Exact-sum selection remains cardinality-bounded.
...
constexpr std::string_view kCandidateSchemaV2 = kTrustedTrajectoryV2ContractId;
constexpr std::string_view kFrameSchemaV2 = kTrustedTrajectoryV2ContractId;
constexpr std::string_view kClosureSchemaV2 = kTrustedTrajectoryV2ContractId;
constexpr std::string_view kManifestSchemaV2 = kTrustedTrajectoryV2ContractId;
constexpr std::string_view kEnvelopeSchemaV2 = kTrustedTrajectoryV2ContractId;

bool valid_v2_card_selection_operation(const std::uint8_t value) noexcept {
    return value <= static_cast<std::uint8_t>(
                        environment::PublicCardSelectionOperation::Unselect);
}

void validate_v2_candidate(const environment::EnvironmentActionCandidate& value) {
    if (!valid_environment_action_kind(static_cast<std::uint8_t>(value.action_kind)) ||
        value.action_kind == environment::EnvironmentActionKind::Unsupported ||
        value.public_action_key.empty() ||
        !valid_v2_card_selection_operation(
            static_cast<std::uint8_t>(value.card_selection_operation))) {
        throw std::invalid_argument("V2 trajectory candidate is unsupported or malformed");
    }
    if (!value.continuation_operation.empty() && value.continuation_operation != "pick" &&
        value.continuation_operation != "amount" && value.continuation_operation != "finish" &&
        value.continuation_operation != "cancel" && value.continuation_operation != "bypass") {
        throw std::invalid_argument("V2 trajectory candidate continuation operation is unknown");
    }
    if (value.action_kind != environment::EnvironmentActionKind::CardSelection &&
        ...
        throw std::invalid_argument("non-card V2 candidate carries selection operation");
    }
    if ((value.action_kind == environment::EnvironmentActionKind::Finish ||
         value.action_kind == environment::EnvironmentActionKind::Cancel) &&
        value.card_selection_operation != environment::PublicCardSelectionOperation::None) {
        throw std::invalid_argument("finish or cancel V2 candidate carries selection operation");
    }
    environment::PublicActionKeyInput input;
    input.action_kind = std::string(environment::environment_action_kind_name(value.action_kind));
    input.choice = value.choice;
    ...
    if (environment::public_action_key_v2(input) != value.public_action_key ||
        !environment::is_public_action_key_v2(value.public_action_key)) {
        throw std::invalid_argument("V2 trajectory public action key is not canonical");
    }
}

void validate_v2_successor(const Successor& value) {
    validate_successor(value);
}

void write_public_candidate_v2(
    ByteWriter& writer, const environment::EnvironmentActionCandidate& value) {
    validate_v2_candidate(value);
    writer.string(kCandidateSchemaV2);
    writer.string(environment::environment_action_kind_name(value.action_kind));
...
bool read_public_candidate_v2(
    ByteReader& reader, environment::EnvironmentActionCandidate& value) noexcept {
    std::string schema;
    std::string action_token;
    std::uint8_t operation = 0;
    if (!reader.string(schema) || schema != kCandidateSchemaV2 ||
        !reader.string(action_token) || !reader.u8(operation) ||
        !valid_v2_card_selection_operation(operation)) {
        return false;
    }
    const auto kind = action_kind_from_token(action_token);
    if (!kind.has_value() || !reader.string(value.public_action_key) ||
...
void validate_v2_request(const environment::EnvironmentDecisionRequest& value) {
    if (value.player > 1 ||
        !valid_environment_decision_kind(static_cast<std::uint8_t>(value.kind)) ||
        value.kind == environment::EnvironmentDecisionKind::Unsupported ||
        value.candidates.empty()) {
        throw std::invalid_argument("V2 trajectory request is invalid or incomplete");
    }
    std::vector<std::string> keys;
    keys.reserve(value.candidates.size());
    for (const auto& candidate : value.candidates) {
    ...
    for (std::size_t left = 0; left < keys.size(); ++left) {
        for (std::size_t right = left + 1; right < keys.size(); ++right) {
            if (keys[left] == keys[right]) {
                throw std::invalid_argument("V2 request has duplicate public action keys");
            }
        }
    }
    if (!value.continuation.has_value()) {
        for (const auto& candidate : value.candidates) {
            if (!candidate.continuation_operation.empty() ||
                !candidate.submits_engine_response) {
                throw std::invalid_argument("V2 atomic request has invalid candidate");
            }
        }
        return;
    }
    ...
    for (const auto& candidate : value.candidates) {
        if (candidate.continuation_operation.empty()) {
            throw std::invalid_argument("V2 continuation candidate has no operation");
        }
        if (candidate.continuation_operation == "pick") {
            if (candidate.action_kind != environment::EnvironmentActionKind::Pick ||
                candidate.submits_engine_response || continuation.continuation_kind == "counter" ||
                !candidate.source_index.has_value() ||
                !is_remaining_index(*candidate.source_index)) {
                throw std::invalid_argument("V2 pick candidate is structurally invalid");
            }
            if (continuation.continuation_kind == "ordering") {
                ordering_picks.push_back(*candidate.source_index);
            }
            ...
            if (candidate.action_kind != environment::EnvironmentActionKind::AssignAmount ||
                ...
                *candidate.amount < 0 || !is_remaining_index(*candidate.source_index) ||
                continuation.remaining_indices.empty() ||
                *candidate.source_index != continuation.remaining_indices.front()) {
                throw std::invalid_argument("V2 amount candidate is structurally invalid");
            }
        } else if (candidate.continuation_operation == "finish") {
            ++finish_count;
            if (candidate.action_kind != environment::EnvironmentActionKind::Finish ||
                !candidate.submits_engine_response || !continuation.can_finish) {
                throw std::invalid_argument("V2 finish candidate is structurally invalid");
            }
        } else if (candidate.continuation_operation == "cancel") {
            ++cancel_count;
            if (candidate.action_kind != environment::EnvironmentActionKind::Cancel ||
                !candidate.submits_engine_response || !continuation.can_cancel) {
                throw std::invalid_argument("V2 cancel candidate is structurally invalid");
            }
        } else if (candidate.continuation_operation == "bypass") {
            ++bypass_count;
            if (candidate.action_kind != environment::EnvironmentActionKind::Cancel ||
                ...
                throw std::invalid_argument("V2 bypass candidate is structurally invalid");
            }
        } else {
            throw std::invalid_argument("V2 continuation candidate has unknown operation");
        }
    }
    if (continuation.continuation_kind == "ordering" &&
        (bypass_count != 1 || ordering_picks != continuation.remaining_indices)) {
        throw std::invalid_argument("V2 ordering domain is incomplete");
    }
    if (finish_count > 1 || cancel_count > 1 || bypass_count > 1 ||
        (finish_count != 0) != continuation.can_finish ||
        (cancel_count != 0) != continuation.can_cancel) {
        throw std::invalid_argument("V2 continuation terminal candidates disagree with state");
    }
}

void write_request_v2(
    ...
    writer.u32be(static_cast<std::uint32_t>(value.candidates.size()));
    for (const auto& candidate : value.candidates) {
        write_public_candidate_v2(writer, candidate);
    }
    writer.u8(value.continuation.has_value() ? 1 : 0);
    if (value.continuation.has_value()) {
        write_continuation_v2(writer, *value.continuation);
    }
}

bool read_request_v2_direct(
    ...
    for (std::uint32_t index = 0; index < count; ++index) {
        environment::EnvironmentActionCandidate candidate;
        if (!read_public_candidate_v2(reader, candidate)) {
            return false;
        }
        value.candidates.push_back(std::move(candidate));
    }
    ...
    if (present == 1) {
        environment::EnvironmentContinuationView continuation;
        if (!read_continuation_v2(reader, continuation)) {
            return false;
        }
        value.continuation = std::move(continuation);
    } else {
...
void validate_v2_frame(const PublicFrameSnapshotV2& value) {
    require_contract(value.episodic_environment_contract_id,
                     environment::kEpisodicEnvironmentV3ContractId,
                     "V3 contract");
    require_digest(value.episode_semantic_id, "episode");
    validate_v2_request(value.request);
    if (value.acting_player > 1 || value.request.player != value.acting_player ||
        value.public_observation.perspective_player != value.acting_player ||
        value.public_observation.decision_index != value.decision_index) {
        throw std::invalid_argument("V2 frame perspective or index mismatch");
    }
    if (environment::public_observation_digest(value.public_observation) !=
        value.public_observation_digest) {
        throw std::invalid_argument("V2 frame observation digest mismatch");
    }
    std::vector<std::string> keys;
    keys.reserve(value.request.candidates.size());
    for (const auto& candidate : value.request.candidates) {
    ...
    if (environment::public_candidate_domain_digest_v2(
            std::string(environment::environment_decision_kind_name(value.request.kind)), keys) !=
        value.public_candidate_domain_digest) {
        throw std::invalid_argument("V2 frame candidate digest mismatch");
    }
    environment::PublicSemanticDecisionIdentityInput identity;
    identity.episode_semantic_id = value.episode_semantic_id;
    identity.decision_index = value.decision_index;
    ...
    identity.public_observation_digest = value.public_observation_digest;
    identity.public_candidate_domain_digest = value.public_candidate_domain_digest;
    if (environment::public_semantic_decision_id_v2(identity) !=
        value.public_semantic_decision_id) {
        throw std::invalid_argument("V2 frame decision identity mismatch");
    }
}

bool read_frame_v2_direct(ByteReader& reader, PublicFrameSnapshotV2& value) noexcept {
    std::string schema;
    if (!reader.string(schema) || schema != kFrameSchemaV2 ||
        ...
        !reader.string(value.public_observation_digest) ||
        !read_request_v2_direct(reader, value.request) ||
        !reader.string(value.public_candidate_domain_digest)) {
        return false;
    }
    try {
        validate_v2_frame(value);
    } catch (...) {
        return false;
    }
    return true;
...
void validate_v2_public_record(const DecisionRecordV2& value) {
    validate_v2_frame(value.frame);
    validate_v2_successor(value.successor);
    if (!environment::is_public_action_key_v2(value.selected_public_action_key)) {
        throw std::invalid_argument("V2 selected action key is invalid");
    }
    std::size_t matches = 0;
    const environment::EnvironmentActionCandidate* selected = nullptr;
    for (const auto& candidate : value.frame.request.candidates) {
    ...
    if (matches != 1 || selected == nullptr) {
        throw std::invalid_argument("V2 selection is not exactly one candidate");
    }
    const auto transition = static_cast<std::uint8_t>(value.transition_class);
    if (!valid_transition_class(transition)) {
        throw std::invalid_argument("V2 transition class is unknown");
    }
    if (value.transition_class == TransitionClass::AtomicEngineResponse) {
        if (value.frame.request.continuation.has_value() || !selected->submits_engine_response) {
            throw std::invalid_argument("V2 atomic transition is inconsistent");
        }
    } else if (value.transition_class == TransitionClass::IntermediateContinuation) {
        if (!value.frame.request.continuation.has_value() || selected->submits_engine_response) {
            throw std::invalid_argument("V2 intermediate transition is inconsistent");
        }
    } else if (value.transition_class == TransitionClass::FinalContinuationResponse) {
        if (!value.frame.request.continuation.has_value() || !selected->submits_engine_response) {
            throw std::invalid_argument("V2 final transition is inconsistent");
        }
    }
    if (value.successor.kind == SuccessorKind::NextFrame &&
        (!value.successor.next_frame.has_value() ||
         value.frame.decision_index == std::numeric_limits<std::uint64_t>::max() ||
         value.successor.next_frame->next_decision_index != value.frame.decision_index + 1)) {
        throw std::invalid_argument("V2 successor index is inconsistent");
    }
}

bool read_public_record_v2_direct(ByteReader& reader, DecisionRecordV2& value) noexcept {
    std::string schema;
    std::uint8_t transition = 0;
    if (!reader.string(schema) || schema != kTrustedTrajectoryV2ContractId ||
        !read_frame_v2_direct(reader, value.frame) ||
    ...
    value.transition_class = static_cast<TransitionClass>(transition);
    try {
        validate_v2_public_record(value);
    } catch (...) {
        return false;
    }
    return true;
...
std::vector<std::uint8_t> canonical_public_environment_action_candidate_bytes_v2(
    const environment::EnvironmentActionCandidate& value) {
    ByteWriter writer;
    write_public_candidate_v2(writer, value);
    return std::move(writer).take();
}
...
decode_public_environment_action_candidate_v2(
    ...
    try {
        ByteReader reader(bytes);
        environment::EnvironmentActionCandidate value;
        if (!read_public_candidate_v2(reader, value) || !reader.at_end() ||
            canonical_public_environment_action_candidate_bytes_v2(value) != bytes) {
            return failure<environment::EnvironmentActionCandidate>(
                "malformed or noncanonical V2 candidate");
        }
        return success(std::move(value));
    } catch (const std::exception& error) {
        return failure<environment::EnvironmentActionCandidate>(error.what());
    } catch (...) {
        return failure<environment::EnvironmentActionCandidate>("V2 candidate decode threw");
    }
}
...
decode_public_environment_decision_request_v2(
    ...
    try {
        ByteReader reader(bytes);
        environment::EnvironmentDecisionRequest value;
        if (!read_request_v2_direct(reader, value) || !reader.at_end() ||
            canonical_public_environment_decision_request_bytes_v2(value) != bytes) {
            return failure<environment::EnvironmentDecisionRequest>(
                "malformed or noncanonical V2 request");
        }
        return success(std::move(value));
    } catch (const std::exception& error) {
        return failure<environment::EnvironmentDecisionRequest>(error.what());
    } catch (...) {
        return failure<environment::EnvironmentDecisionRequest>("V2 request decode threw");
    }
}
...
std::vector<std::uint8_t> canonical_public_frame_snapshot_bytes_v2(
    ...
    writer.string(value.public_observation_digest);
    writer.raw(canonical_public_environment_decision_request_bytes_v2(value.request));
    writer.string(value.public_candidate_domain_digest);
    return std::move(writer).take();
}
...
DecodeResult<PublicFrameSnapshotV2> decode_public_frame_snapshot_v2(
    ...
    try {
        ...
        if (!read_frame_v2_direct(reader, value) || !reader.at_end() ||
            canonical_public_frame_snapshot_bytes_v2(value) != bytes) {
            return failure<PublicFrameSnapshotV2>("malformed or noncanonical V2 frame");
        }
        return success(std::move(value));
    } catch (const std::exception& error) {
        return failure<PublicFrameSnapshotV2>(error.what());
    } catch (...) {
        return failure<PublicFrameSnapshotV2>("V2 frame decode threw");
    }
}
...
std::vector<std::uint8_t> canonical_public_decision_record_bytes_v2(
    ...
    writer.raw(canonical_public_frame_snapshot_bytes_v2(value.frame));
    writer.string(value.selected_public_action_key);
    writer.u8(static_cast<std::uint8_t>(value.transition_class));
    write_successor(writer, value.successor);
    return std::move(writer).take();
...
DecodeResult<DecisionRecordV2> decode_public_decision_record_v2(
    ...
    try {
        ...
        if (!read_public_record_v2_direct(reader, value) || !reader.at_end() ||
            canonical_public_decision_record_bytes_v2(value) != bytes) {
            return failure<DecisionRecordV2>("malformed or noncanonical V2 public record");
        }
        return success(std::move(value));
    } catch (const std::exception& error) {
        return failure<DecisionRecordV2>(error.what());
    } catch (...) {
        return failure<DecisionRecordV2>("V2 public record decode threw");
    }
}

namespace {

void validate_v2_decision_record(const DecisionRecordV2& value) {
    validate_v2_public_record(value);
    require_identity(value.acting_policy_assignment_id,
                     "participant_policy_assignment.v1.", "V2 record assignment");
    validate_rng_decision(value.policy_rng_decision_provenance);
    if (value.policy_rng_decision_provenance.decision_index != value.frame.decision_index ||
        value.policy_rng_decision_provenance.acting_policy_assignment_id !=
            value.acting_policy_assignment_id) {
        throw std::invalid_argument("V2 record attribution does not match frame");
    }
}
...
bool read_collection_record_v2_direct(ByteReader& reader, DecisionRecordV2& value) noexcept {
    std::string schema;
    std::string attribution_schema;
    if (!reader.string(schema) || schema != kTrustedTrajectoryV2ContractId ||
        !read_public_record_v2_direct(reader, value) ||
...
void validate_v2_terminal_closure(const TerminalClosureV2& value) {
    if (value.winner > 2 || value.win_reason == 255 ||
        (value.semantic_action_count == 0
             ...
             : !value.last_decision_index.has_value() ||
                   *value.last_decision_index == std::numeric_limits<std::uint64_t>::max() ||
                   *value.last_decision_index + 1 != value.semantic_action_count)) {
        throw std::invalid_argument("V2 terminal closure is inconsistent");
    }
    if (environment::public_observation_digest(value.terminal_view_player_0) !=
            value.terminal_view_player_0_digest ||
        environment::public_observation_digest(value.terminal_view_player_1) !=
            value.terminal_view_player_1_digest) {
        throw std::invalid_argument("V2 terminal observation digest mismatch");
    }
}

void validate_v2_interrupted_closure(const InterruptedClosureV2& value) {
    if (value.pending_unacted_frame.has_value()) {
        if (value.pending_unacted_frame->decision_index != value.record_count) {
            throw std::invalid_argument("V2 pending frame index is inconsistent");
        }
        validate_v2_frame(*value.pending_unacted_frame);
    }
}

void validate_v2_failed_closure(const FailedClosureV2& value) {
    if (!valid_failure_code(static_cast<std::uint8_t>(value.failure_code)) ||
        !valid_failure_stage(static_cast<std::uint8_t>(value.failure_stage))) {
        throw std::invalid_argument("V2 failed closure has an unknown failure value");
    }
}

void validate_v2_closure(const EpisodeClosureV2& value) {
    std::visit(
        [](const auto& closure) {
            using T = std::decay_t<decltype(closure)>;
            if constexpr (std::is_same_v<T, TerminalClosureV2>) {
                validate_v2_terminal_closure(closure);
            } else if constexpr (std::is_same_v<T, InterruptedClosureV2>) {
                validate_v2_interrupted_closure(closure);
            } else {
                validate_v2_failed_closure(closure);
            }
        },
        value);
}

void write_closure_v2(ByteWriter& writer, const EpisodeClosureV2& value) {
    validate_v2_closure(value);
    writer.string(kClosureSchemaV2);
    if (const auto* terminal = std::get_if<TerminalClosureV2>(&value)) {
        writer.u8(0);
        writer.u8(terminal->winner);
        writer.u8(terminal->win_reason);
        writer.u64be(terminal->semantic_action_count);
        ...
        writer.raw(environment::canonical_public_environment_observation_bytes(
            terminal->terminal_view_player_1));
        writer.string(terminal->terminal_view_player_1_digest);
    } else if (const auto* interrupted = std::get_if<InterruptedClosureV2>(&value)) {
        writer.u8(1);
        writer.u64be(interrupted->record_count);
        writer.u8(interrupted->pending_unacted_frame.has_value() ? 1 : 0);
        if (interrupted->pending_unacted_frame.has_value()) {
            writer.raw(canonical_public_frame_snapshot_bytes_v2(
                *interrupted->pending_unacted_frame));
        }
    } else {
        const auto& failed = std::get<FailedClosureV2>(value);
        writer.u8(2);
        writer.u8(static_cast<std::uint8_t>(failed.failure_code));
        writer.u8(static_cast<std::uint8_t>(failed.failure_stage));
        writer.boolean(failed.mutation_may_have_occurred);
...
bool read_closure_v2_direct(ByteReader& reader, EpisodeClosureV2& value) noexcept {
    std::string schema;
    std::uint8_t kind = 0;
    if (!reader.string(schema) || schema != kClosureSchemaV2 || !reader.u8(kind) ||
        !valid_closure_kind(kind)) {
        return false;
    }
    if (kind == 0) {
        ...
        if (present == 1) {
            ...
            if (!read_frame_v2_direct(reader, frame)) {
                return false;
            }
            interrupted.pending_unacted_frame = std::move(frame);
        }
...
void validate_v2_manifest(const EpisodeManifestV2& value) {
    require_contract(value.trusted_trajectory_contract_id, kTrustedTrajectoryV2ContractId,
                     "V2 trusted trajectory contract");
    require_contract(value.episodic_environment_contract_id,
                     environment::kEpisodicEnvironmentV3ContractId,
    ...
    if (value.environment_identity_input.empty() || value.episode_identity_input.empty() ||
        ...
        trace::sha256_bytes(value.episode_identity_input) != value.episode_semantic_id) {
        throw std::invalid_argument("V2 manifest identity input mismatch");
    }
    const auto environment_result =
        decode_environment_identity_input_v3(value.environment_identity_input);
    if (!environment_result ||
        environment_result.value->environment_semantic_id != value.environment_semantic_id ||
        !is_current_certified_environment_v3(*environment_result.value)) {
        throw std::invalid_argument("V2 manifest environment identity is not current V3");
    }
    const auto episode_result = decode_episode_identity_input_v3(
    ...
    if (!episode_result ||
        environment::episode_semantic_id(*environment_result.value, *episode_result.value) !=
            value.episode_semantic_id) {
        throw std::invalid_argument("V2 manifest episode identity is not canonical V3");
    }
    (void)canonical_policy_provenance_envelope_bytes(value.policy_provenance);
    (void)canonical_collection_disposition_bytes(value.collection_disposition);
}
...
void validate_v2_envelope_sequence(const EpisodeEnvelopeV2& value) {
    validate_v2_manifest(value.manifest);
    require_length(value.records.size());
    for (std::size_t index = 0; index < value.records.size(); ++index) {
        const auto& record = value.records[index];
        if (record.frame.episode_semantic_id != value.manifest.episode_semantic_id ||
            record.frame.decision_index != index) {
            throw std::invalid_argument("V2 envelope record identity or index mismatch");
        }
        validate_v2_decision_record(record);
        if (index > 0) {
            const auto& previous = value.records[index - 1];
            if (previous.successor.kind != SuccessorKind::NextFrame ||
                !previous.successor.next_frame.has_value() ||
                ...
                previous.successor.next_frame->next_public_semantic_decision_id !=
                    record.frame.public_semantic_decision_id) {
                throw std::invalid_argument("V2 envelope successor sequence is inconsistent");
            }
        }
    }
    const auto count = static_cast<std::uint64_t>(value.records.size());
    if (const auto* terminal = std::get_if<TerminalClosureV2>(&value.closure)) {
        if (terminal->semantic_action_count != count ||
            (count == 0 ? terminal->last_decision_index.has_value()
                        : !terminal->last_decision_index.has_value() ||
                              *terminal->last_decision_index == std::numeric_limits<std::uint64_t>::max() ||
                              *terminal->last_decision_index + 1 != count)) {
            throw std::invalid_argument("V2 terminal envelope count is inconsistent");
        }
        if (!value.records.empty() &&
            value.records.back().successor.kind != SuccessorKind::Terminal) {
            throw std::invalid_argument("V2 terminal envelope has no terminal successor");
        }
    } else if (const auto* interrupted = std::get_if<InterruptedClosureV2>(&value.closure)) {
        if (interrupted->record_count != count) {
            throw std::invalid_argument("V2 interrupted envelope count is inconsistent");
        }
        if (interrupted->pending_unacted_frame.has_value()) {
            ...
            if (pending.episode_semantic_id != value.manifest.episode_semantic_id ||
                ...
                throw std::invalid_argument("V2 pending interruption is inconsistent");
            }
        } else if (!value.records.empty() &&
                   value.records.back().successor.kind != SuccessorKind::Interrupted) {
            throw std::invalid_argument("V2 interrupted envelope has no interruption successor");
        }
    } else {
        const auto& failed = std::get<FailedClosureV2>(value.closure);
        if (failed.record_count != count) {
            throw std::invalid_argument("V2 failed envelope count is inconsistent");
        }
        if (!value.records.empty() &&
            value.records.back().successor.kind != SuccessorKind::Failed) {
            throw std::invalid_argument("V2 failed envelope has no failed successor");
        }
    }
}
...
DecodeResult<DecisionRecordV2> decode_collection_decision_record_v2(
    ...
    try {
        ByteReader reader(bytes);
        DecisionRecordV2 value;
        if (!read_collection_record_v2_direct(reader, value) || !reader.at_end() ||
            canonical_collection_decision_record_bytes_v2(value) != bytes) {
            return failure<DecisionRecordV2>("malformed or noncanonical V2 collection record");
        }
        return success(std::move(value));
    } catch (const std::exception& error) {
        return failure<DecisionRecordV2>(error.what());
    } catch (...) {
        return failure<DecisionRecordV2>("V2 collection record decode threw");
    }
}
...
DecodeResult<EpisodeClosureV2> decode_episode_closure_v2(
    ...
    try {
        ByteReader reader(bytes);
        EpisodeClosureV2 value;
        if (!read_closure_v2_direct(reader, value) || !reader.at_end() ||
            canonical_episode_closure_bytes_v2(value) != bytes) {
            return failure<EpisodeClosureV2>("malformed or noncanonical V2 closure");
        }
        return success(std::move(value));
    } catch (const std::exception& error) {
        return failure<EpisodeClosureV2>(error.what());
    } catch (...) {
        return failure<EpisodeClosureV2>("V2 closure decode threw");
    }
}
...
DecodeResult<EpisodeManifestV2> decode_episode_manifest_v2(
    ...
    try {
        ...
        if (!read_manifest_v2_direct(reader, value) || !reader.at_end() ||
            canonical_episode_manifest_bytes_v2(value) != bytes) {
            return failure<EpisodeManifestV2>("malformed or noncanonical V2 manifest");
        }
        return success(std::move(value));
    } catch (const std::exception& error) {
        return failure<EpisodeManifestV2>(error.what());
    } catch (...) {
        return failure<EpisodeManifestV2>("V2 manifest decode threw");
    }
}

std::vector<std::uint8_t> canonical_episode_envelope_bytes_v2(
    const EpisodeEnvelopeV2& value) {
    validate_v2_envelope_sequence(value);
    ByteWriter writer;
    writer.string(kEnvelopeSchemaV2);
    writer.raw(canonical_episode_manifest_bytes_v2(value.manifest));
    require_length(value.records.size());
    writer.u32be(static_cast<std::uint32_t>(value.records.size()));
    for (const auto& record : value.records) {
        writer.raw(canonical_collection_decision_record_bytes_v2(record));
    }
...
DecodeResult<EpisodeEnvelopeV2> decode_episode_envelope_v2(
    ...
    try {
        ...
        std::string schema;
        std::uint32_t count = 0;
        if (!reader.string(schema) || schema != kEnvelopeSchemaV2 ||
            !read_manifest_v2_direct(reader, value.manifest) || !reader.u32be(count) ||
            count > reader.remaining()) {
            return failure<EpisodeEnvelopeV2>("malformed V2 envelope header");
        }
        value.records.reserve(count);
        for (std::uint32_t index = 0; index < count; ++index) {
            DecisionRecordV2 record;
            if (!read_collection_record_v2_direct(reader, record)) {
                return failure<EpisodeEnvelopeV2>("malformed V2 collection record");
            }
            value.records.push_back(std::move(record));
        }
        if (!read_closure_v2_direct(reader, value.closure) || !reader.at_end()) {
            return failure<EpisodeEnvelopeV2>("malformed V2 closure or trailing bytes");
        }
        validate_v2_envelope_sequence(value);
        if (canonical_episode_envelope_bytes_v2(value) != bytes) {
            return failure<EpisodeEnvelopeV2>("noncanonical V2 envelope");
        }
        return success(std::move(value));
    } catch (const std::exception& error) {
        return failure<EpisodeEnvelopeV2>(error.what());
    } catch (...) {
        return failure<EpisodeEnvelopeV2>("V2 envelope decode threw");
    }
}

namespace {

void validate_v2_restricted_evidence(const RestrictedReplayEvidenceV2& value) {
    require_contract(value.restricted_replay_evidence_contract_id,
                     kRestrictedReplayEvidenceV2SchemaId,
                     "V2 restricted evidence schema");
    require_contract(value.trusted_trajectory_contract_id, kTrustedTrajectoryV2ContractId,
                     "V2 restricted evidence trajectory");
    require_contract(value.episodic_environment_contract_id,
                     environment::kEpisodicEnvironmentV3ContractId,
                     "V2 restricted evidence environment");
    require_digest(value.episode_semantic_id, "V2 restricted evidence episode");
    if (value.closure_kind != 1 ||
        !valid_interruption_reason(static_cast<std::uint8_t>(value.interruption_reason)) ||
        value.engine_process_budget == 0 || value.semantic_action_budget == 0 ||
        value.observed_engine_process_count > value.engine_process_budget ||
        value.observed_semantic_action_count > value.semantic_action_budget) {
        throw std::invalid_argument("V2 restricted evidence is inconsistent");
    }
}
...
std::vector<std::uint8_t> canonical_restricted_replay_evidence_bytes_v2(
    ...
    validate_v2_restricted_evidence(value);
    ByteWriter writer;
    writer.string(kRestrictedReplayEvidenceV2SchemaId);
    writer.string(value.trusted_trajectory_contract_id);
    writer.string(value.episodic_environment_contract_id);
    writer.string(value.episode_semantic_id);
...
DecodeResult<RestrictedReplayEvidenceV2> decode_restricted_replay_evidence_v2(
    ...
    try {
        ...
        if (!reader.string(value.restricted_replay_evidence_contract_id) ||
            ...
            !reader.u64be(value.observed_semantic_action_count) ||
            !reader.u64be(value.final_engine_step_index) || !reader.at_end()) {
            return failure<RestrictedReplayEvidenceV2>(
                "malformed V2 restricted evidence");
        }
        value.interruption_reason =
            static_cast<environment::InterruptionReason>(reason);
        validate_v2_restricted_evidence(value);
        if (canonical_restricted_replay_evidence_bytes_v2(value) != bytes) {
            return failure<RestrictedReplayEvidenceV2>(
                "noncanonical V2 restricted evidence");
        }
        return success(std::move(value));
    } catch (const std::exception& error) {
        return failure<RestrictedReplayEvidenceV2>(error.what());
    } catch (...) {
        return failure<RestrictedReplayEvidenceV2>("V2 restricted evidence decode threw");
    }
}
}  // namespace ygo::trajectory
