...
void validate_logical(const LogicalModelInputV2& logical) {
    ...
    for (std::size_t index = 0; index < logical.candidate_features.size(); ++index) {
        ...
        try {
            if (ygo::environment::public_action_key_v3(key) != key_value) {
                fail_logical();
            }
...
void write_logical_candidate_exact(Writer& writer, const LogicalCandidateV2& candidate,
                                   const std::string& public_action_key) {
    writer.string(action_kind_token(candidate.action_kind));
    writer.u8(static_cast<std::uint8_t>(candidate.card_selection_operation));
...
std::vector<std::uint8_t> canonical_logical_bytes_unchecked(
    ...
    Writer writer;
    writer.string(kLogicalModelInputV2SchemaId);
    writer.string(kLogicalModelInputV2SchemaId);
...
EncodedModelInputV2 encode_unchecked(const LogicalModelInputV2& logical,
                                     const CardVocabularyV1& vocabulary) {
    EncodedModelInputV2 output;
    output.schema_id = std::string(kEncodedModelInputV2SchemaId);
    ...
    for (const auto& source : logical.candidate_features) {
        EncodedCandidateV2 candidate;
        candidate.action_kind_code = action_kind_code(source.action_kind);
        candidate.card_selection_operation_code = static_cast<std::uint8_t>(source.card_selection_operation);
...
void validate_encoded_candidate_routing(
    const EncodedCandidateV2& candidate,
    ...
    key.card_selection_operation = static_cast<ygo::environment::PublicCardSelectionOperation>(candidate.card_selection_operation_code);
    if (candidate.choice.has_value()) {
    ...
    try {
        if (ygo::environment::public_action_key_v3(key) != routing_key) {
            fail_encoded();
        }
    } catch (const EncodingFailure&) {
        throw;
...
void validate_encoded(const EncodedModelInputV2& encoded) {
    ...
    for (const auto& key : encoded.routing_keys) {
        if (!ygo::environment::is_public_action_key_v3(key) ||
...
void write_encoded(Writer& writer, const EncodedModelInputV2& encoded) {
    writer.string(kEncodedModelInputV2SchemaId);
    writer.string(kEncodedModelInputV2SchemaId);
    writer.string(kLogicalModelInputV2SchemaId);
    ...
    for (const auto& candidate : encoded.candidate_features) {
        ...
        writer.u8(candidate.card_selection_operation_code);
        writer.boolean(candidate.choice.has_value());
        if (candidate.choice.has_value()) {
...
std::string_view encoded_model_input_error_code_name(
    const EncodedModelInputErrorCodeV2 code) noexcept {
    switch (code) {
    case EncodedModelInputErrorCodeV2::InvalidLogicalModelInput:
        return "invalid_logical_model_input";
    case EncodedModelInputErrorCodeV2::UnknownPublicPasscode:
        return "unknown_public_passcode";
    case EncodedModelInputErrorCodeV2::InvalidEncodedModelInput:
        return "invalid_encoded_model_input";
    case EncodedModelInputErrorCodeV2::InternalFailure:
...
EncodedModelInputResultV2 encode_model_input_v2(
    ...
    try {
        ...
        return {std::optional<EncodedModelInputV2>(std::move(encoded)), std::nullopt};
        ...
        if (failure.reason() == FailureReason::InvalidLogical) {
            code = EncodedModelInputErrorCodeV2::InvalidLogicalModelInput;
        } else if (failure.reason() == FailureReason::UnknownPublicPasscode) {
            code = EncodedModelInputErrorCodeV2::UnknownPublicPasscode;
        } else if (failure.reason() == FailureReason::InvalidEncoded) {
            code = EncodedModelInputErrorCodeV2::InvalidEncodedModelInput;
        }
        ...
        return {std::nullopt, EncodedModelInputErrorV2{code, diagnostic}};
    } catch (const std::bad_alloc&) {
        return {std::nullopt, EncodedModelInputErrorV2{
                                   EncodedModelInputErrorCodeV2::InternalFailure,
...
std::vector<std::uint8_t> canonical_model_input_identity_bytes_v2(
    ...
    Writer writer;
    writer.string(kModelInputIdentityV2SchemaId);
    writer.string(kModelInputIdentityV2SchemaId);
    writer.string(kLogicalModelInputV2SchemaId);
    writer.string(kEncodedModelInputV2SchemaId);
    writer.string(encoded.card_vocabulary_identity);
...
std::string model_input_identity_v2(const LogicalModelInputV2& logical,
                                 const EncodedModelInputV2& encoded) {
    return std::string(kModelInputIdentityV2Prefix) +
           ygo::trace::sha256_bytes(canonical_model_input_identity_bytes_v2(logical, encoded));
}
