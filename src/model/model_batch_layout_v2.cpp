...
public:
    explicit LayoutFailure(const ModelBatchLayoutErrorCodeV2 code) : code_(code) {}

    ModelBatchLayoutErrorCodeV2 code() const noexcept { return code_; }

private:
    ModelBatchLayoutErrorCodeV2 code_;
};

[[noreturn]] void fail(const ModelBatchLayoutErrorCodeV2 code) {
...
std::size_t checked_product(const std::uint32_t batch_size,
    ...
    if (width > std::numeric_limits<std::size_t>::max() ||
        ...
        fail(ModelBatchLayoutErrorCodeV2::CountOverflow);
    }
...
std::uint64_t max_collection_length(const std::vector<std::uint64_t>& offsets) {
    if (offsets.size() < 2) {
        fail(ModelBatchLayoutErrorCodeV2::InvalidRaggedLayout);
    }
...
void validate_header(const ModelBatchSampleHeaderV2& sample) {
    if (sample.schema_id != kEncodedModelInputV2SchemaId ||
...
ModelBatchSampleHeaderV2 make_header(const EncodedModelInputV2& sample) {
    ModelBatchSampleHeaderV2 header;
...
CandidateOptionalPresenceV2 candidate_presence(const EncodedCandidateV2& candidate) {
...
bool same_presence(const CandidateOptionalPresenceV2& left,
                   const CandidateOptionalPresenceV2& right) noexcept {
...
EncodedModelInputV2 reconstruct_unchecked(const RaggedModelBatchV2& ragged,
...
void validate_ragged(const RaggedModelBatchV2& ragged) {
    if (ragged.schema_id != kModelBatchLayoutV2SchemaId || ragged.batch_size == 0 ||
        ragged.samples.size() != ragged.batch_size) {
        fail(ModelBatchLayoutErrorCodeV2::EmptyBatch);
    }
    ...
    for (std::size_t index = 0; index < ragged.candidate_rows.size(); ++index) {
        if (!same_presence(ragged.candidate_optional_presence_masks[index],
            ...
            fail(ModelBatchLayoutErrorCodeV2::OptionalPresenceMismatch);
        }
    }
    for (std::size_t index = 0; index < ragged.samples.size(); ++index) {
        try {
            ...
            fail(ModelBatchLayoutErrorCodeV2::InvalidEncodedSample);
        } catch (...) {
            fail(ModelBatchLayoutErrorCodeV2::InvalidEncodedSample);
        }
    }
...
void collect_padded_collection(const std::vector<T>& padded_values,
    ...
    if (padded_values.size() != expected_size || row_mask.size() != expected_size) {
        fail(ModelBatchLayoutErrorCodeV2::InvalidPaddedLayout);
    }
    offsets.clear();
    offsets.push_back(0);
    flat_values.clear();
    for (std::uint32_t sample = 0; sample < batch_size; ++sample) {
        ...
        for (std::uint64_t column = 0; column < width; ++column) {
            ...
            if (mask > 1) fail(ModelBatchLayoutErrorCodeV2::MaskMismatch);
            if (mask == 1) {
                if (padding_started) fail(ModelBatchLayoutErrorCodeV2::MaskMismatch);
                flat_values.push_back(padded_values[index]);
            } else {
                padding_started = true;
                if (!is_padding(padded_values[index])) {
                    fail(ModelBatchLayoutErrorCodeV2::PaddingValueMismatch);
                }
            }
        }
...
bool is_padding_candidate(const EncodedCandidateV2& value) noexcept {
    return value.action_kind_code == 0 && value.card_selection_operation_code == 0 &&
           !value.choice.has_value() &&
...
void fill_padded_collection(const std::vector<T>& flat_values,
    ...
    for (std::uint32_t sample = 0; sample < batch_size; ++sample) {
        ...
        if (count > width) fail(ModelBatchLayoutErrorCodeV2::CapacityTooSmall);
...
std::uint64_t choose_width(const std::optional<std::uint64_t>& requested,
    ...
    if (width < maximum) fail(ModelBatchLayoutErrorCodeV2::CapacityTooSmall);
    return width;
}

void validate_padded_headers(const PaddedModelBatchV2& padded) {
    if (padded.schema_id != kModelBatchLayoutV2SchemaId || padded.batch_size == 0 ||
...
void validate_padded_sizes(const PaddedModelBatchV2& padded) {
...
const char* diagnostic_for(const ModelBatchLayoutErrorCodeV2 code) noexcept {
    switch (code) {
    case ModelBatchLayoutErrorCodeV2::EmptyBatch:
        return "model batch is empty";
    case ModelBatchLayoutErrorCodeV2::InvalidEncodedSample:
        return "model batch contains an invalid encoded sample";
    case ModelBatchLayoutErrorCodeV2::InvalidRaggedLayout:
        return "ragged model batch layout is invalid";
    case ModelBatchLayoutErrorCodeV2::CapacityTooSmall:
        return "model batch padding capacity is too small";
    case ModelBatchLayoutErrorCodeV2::InvalidPaddedLayout:
        return "padded model batch layout is invalid";
    case ModelBatchLayoutErrorCodeV2::MaskMismatch:
        return "model batch row mask is invalid";
    case ModelBatchLayoutErrorCodeV2::PaddingValueMismatch:
        return "model batch padding row contains a value";
    case ModelBatchLayoutErrorCodeV2::OptionalPresenceMismatch:
        return "model batch optional presence mask is invalid";
    case ModelBatchLayoutErrorCodeV2::RoundtripMismatch:
        return "model batch ragged/padded roundtrip changed a value";
    case ModelBatchLayoutErrorCodeV2::CountOverflow:
        return "model batch count or offset exceeds its declared width";
    case ModelBatchLayoutErrorCodeV2::InternalFailure:
...
template <typename T>
ModelBatchLayoutResultV2<T> failure_with_diagnostic(
    const ModelBatchLayoutErrorCodeV2 code) noexcept {
    ModelBatchLayoutResultV2<T> result;
    result.error = ModelBatchLayoutErrorV2{code, diagnostic_for(code)};
    return result;
}
...
RaggedModelBatchResultV2 make_ragged_model_batch_v2(
    ...
    try {
        if (samples.empty()) return failure_with_diagnostic<RaggedModelBatchV2>(
            ModelBatchLayoutErrorCodeV2::EmptyBatch);
        if (samples.size() > std::numeric_limits<std::uint32_t>::max()) {
            return failure_with_diagnostic<RaggedModelBatchV2>(
                ModelBatchLayoutErrorCodeV2::CountOverflow);
        }
        RaggedModelBatchV2 output;
        ...
        for (const auto& sample : samples) {
            try {
                ...
                return failure_with_diagnostic<RaggedModelBatchV2>(
                    ModelBatchLayoutErrorCodeV2::InvalidEncodedSample);
            }
        ...
        return {std::optional<RaggedModelBatchV2>(std::move(output)), std::nullopt};
    } catch (const LayoutFailure& error) {
        return failure_with_diagnostic<RaggedModelBatchV2>(error.code());
    } catch (const std::bad_alloc&) {
        return failure_with_diagnostic<RaggedModelBatchV2>(
            ModelBatchLayoutErrorCodeV2::InternalFailure);
    } catch (...) {
        return failure_with_diagnostic<RaggedModelBatchV2>(
            ModelBatchLayoutErrorCodeV2::InternalFailure);
    }
...
PaddedModelBatchResultV2 pad_model_batch_v2(
    ...
    try {
        ...
        EncodedCandidateV2 candidate_padding;
        ...
        CandidateOptionalPresenceV2 presence_padding;
        ...
        return {std::optional<PaddedModelBatchV2>(std::move(output)), std::nullopt};
    } catch (const LayoutFailure& error) {
        return failure_with_diagnostic<PaddedModelBatchV2>(error.code());
    } catch (const std::bad_alloc&) {
        return failure_with_diagnostic<PaddedModelBatchV2>(
            ModelBatchLayoutErrorCodeV2::InternalFailure);
    } catch (...) {
        return failure_with_diagnostic<PaddedModelBatchV2>(
            ModelBatchLayoutErrorCodeV2::InternalFailure);
    ...
    try {
        ...
        for (std::size_t index = 0; index < padded.candidate_features_padded.size(); ++index) {
            if (padded.candidate_row_mask[index] == 0) {
                const CandidateOptionalPresenceV2 empty;
                ...
                fail(ModelBatchLayoutErrorCodeV2::MaskMismatch);
            }
        }
        collect_padded_collection(
...
EncodedModelInputV2 reconstruct_model_batch_sample_v2(
    const RaggedModelBatchV2& ragged, const std::size_t sample_index) {
    try {
        validate_ragged(ragged);
...
std::vector<std::uint8_t> canonical_model_batch_layout_bytes_v2(
    ...
    writer.string(kModelBatchLayoutV2SchemaId);
    writer.string(kModelBatchLayoutV2SchemaId);
    writer.u32be(ragged.batch_size);
    for (std::size_t index = 0; index < ragged.batch_size; ++index) {
        const auto sample = reconstruct_model_batch_sample_v2(ragged, index);
        writer.bytes(canonical_encoded_model_input_bytes(sample));
    }
...
std::string model_batch_layout_identity_v2(
    const RaggedModelBatchV2& ragged) {
    return std::string(kModelBatchLayoutV2IdentityPrefix) +
           ygo::trace::sha256_bytes(canonical_model_batch_layout_bytes_v2(ragged));
}
