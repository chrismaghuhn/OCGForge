...
public:
    explicit ProjectionFailure(const LogicalModelProjectionErrorCodeV2 code) : code_(code) {}

    LogicalModelProjectionErrorCodeV2 code() const noexcept { return code_; }

private:
    LogicalModelProjectionErrorCodeV2 code_;
};

[[noreturn]] void fail(const LogicalModelProjectionErrorCodeV2 code) {
    throw ProjectionFailure(code);
}

const char* diagnostic_for(const LogicalModelProjectionErrorCodeV2 code) noexcept {
    switch (code) {
    case LogicalModelProjectionErrorCodeV2::InvalidPublicObservation:
        return "public observation is invalid";
    case LogicalModelProjectionErrorCodeV2::PublicSafeStateDecodeFailure:
        return "public safe state decode failed";
    case LogicalModelProjectionErrorCodeV2::EmptyCandidateDomain:
        return "public candidate domain is empty";
    case LogicalModelProjectionErrorCodeV2::InvalidPublicActionKey:
        return "public action key is invalid";
    case LogicalModelProjectionErrorCodeV2::DuplicatePublicActionKey:
        return "public candidate domain contains a duplicate key";
    case LogicalModelProjectionErrorCodeV2::InvalidPublicCandidateDescriptor:
        return "public candidate descriptor is invalid";
    case LogicalModelProjectionErrorCodeV2::InvalidPublicReference:
        return "public reference is invalid";
    case LogicalModelProjectionErrorCodeV2::CandidateDomainDigestFailure:
        return "public candidate-domain digest could not be derived";
    case LogicalModelProjectionErrorCodeV2::LocatorTableFailure:
        return "public locator table could not be constructed";
    case LogicalModelProjectionErrorCodeV2::InternalFailure:
...
struct LocatorTable final {
    ...
    std::uint32_t ordinal(const std::string_view value) const {
        ...
        if (found == values.end() || *found != value) {
            fail(LogicalModelProjectionErrorCodeV2::LocatorTableFailure);
        }
...
void add_locator_token(std::vector<std::string>& values, const std::string_view value) {
    if (!valid_public_locator(value)) {
        fail(LogicalModelProjectionErrorCodeV2::InvalidPublicReference);
    }
...
void validate_candidate(
    ...
    if (!ygo::environment::is_public_action_key_v3(candidate.public_action_key)) {
        fail(LogicalModelProjectionErrorCodeV2::InvalidPublicActionKey);
    }
    if (!valid_action_kind(candidate.action_kind)) {
        fail(LogicalModelProjectionErrorCodeV2::InvalidPublicCandidateDescriptor);
    }
    if (static_cast<std::uint8_t>(candidate.card_selection_operation) > 2 ||
    ...
    key.card_selection_operation = candidate.card_selection_operation;
    try {
        if (ygo::environment::public_action_key_v3(key) != candidate.public_action_key) {
            fail(LogicalModelProjectionErrorCodeV2::InvalidPublicCandidateDescriptor);
        }
...
LogicalModelProjectionErrorCodeV2 validate_observation_and_decode(
    const ygo::environment::PublicEnvironmentObservation& observation,
    ...
    if (!decoded || !decoded.value.has_value()) {
        return LogicalModelProjectionErrorCodeV2::PublicSafeStateDecodeFailure;
    }
    try {
        if (decoded.value->match_context().perspective_player !=
            observation.perspective_player) {
            return LogicalModelProjectionErrorCodeV2::InvalidPublicObservation;
        }
    ...
    return LogicalModelProjectionErrorCodeV2::InternalFailure;
}
...
void copy_candidates(
    ...
    for (const auto& source : candidates) {
        ...
        if (std::find(keys.begin(), keys.end(), source.public_action_key) !=
            keys.end()) {
            fail(LogicalModelProjectionErrorCodeV2::DuplicatePublicActionKey);
        }
        keys.push_back(source.public_action_key);

        LogicalCandidateV2 candidate;
        ...
        candidate.card_selection_operation = source.card_selection_operation;
...
std::string_view logical_model_projection_error_code_name(
    const LogicalModelProjectionErrorCodeV2 code) noexcept {
    switch (code) {
    case LogicalModelProjectionErrorCodeV2::InvalidPublicObservation:
...
LogicalModelProjectionResultV2 project_logical_model_input_v2(
    ...
    try {
        ...
        if (observation_result != LogicalModelProjectionErrorCodeV2::InternalFailure) {
        ...
        LogicalModelInputV2 output;
        output.schema_id = std::string(kLogicalModelInputV2SchemaId);
        try {
        ...
        return {std::optional<LogicalModelInputV2>(std::move(output)), std::nullopt};
    } catch (const ProjectionFailure& failure) {
        const auto code = failure.code();
        return {std::nullopt, LogicalModelProjectionErrorV2{code, diagnostic_for(code)}};
    } catch (const std::bad_alloc&) {
        const auto code = LogicalModelProjectionErrorCodeV2::InternalFailure;
        return {std::nullopt, LogicalModelProjectionErrorV2{code, diagnostic_for(code)}};
    } catch (...) {
        const auto code = LogicalModelProjectionErrorCodeV2::InternalFailure;
        return {std::nullopt, LogicalModelProjectionErrorV2{code, diagnostic_for(code)}};
