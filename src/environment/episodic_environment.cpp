...
std::string_view rejection_code_name(const RejectionCode code) noexcept {
    ...
    case RejectionCode::WrongSemanticDecision:
        return "WRONG_SEMANTIC_DECISION";
    case RejectionCode::UnknownSemanticKey:
...
const observation::ObservedCard* visible_card_for_reference(
    ...
    if (location == 0) {
        for (const auto& entity : current.entities) {
            if (entity.controller.value_or(2) == controller && entity.identity_known &&
                entity.passcode.value_or(0) == code) {
                return &entity;
            }
        }
        return nullptr;
    ...
    for (const auto& entity : current.entities) {
        if (entity.controller.value_or(2) != controller || entity.zone != projected_zone) {
            continue;
        }
...
bool known_own_deck_identity(const CertifiedEnvironmentConfig& config,
    ...
    if (code == 0 || controller != current.perspective_player ||
        (location != LOCATION_DECK && location != LOCATION_EXTRA) || controller > 1) {
        return false;
    }
    const auto canonical_decks = load_certified_decks();
    if (config.locked_decks.size() != canonical_decks.size() ||
        canonical_decks[0].sha256 != config.locked_decks[0].sha256 ||
        canonical_decks[1].sha256 != config.locked_decks[1].sha256) {
        return false;
    }
    const auto seat_decks = spec.seat_assignment == SeatAssignment::Normal
                                ? canonical_decks
                                : std::vector<core::FixtureDeck>{canonical_decks[1], canonical_decks[0]};
    const auto& cards = location == LOCATION_DECK ? seat_decks[controller].main_deck
                                                   : seat_decks[controller].extra_deck;
    return std::find(cards.begin(), cards.end(), code) != cards.end();
}
...
EnvironmentActionCandidate project_candidate(const protocol::DecisionRequest& request,
                                             const protocol::ActionCandidate& candidate,
                                             const observation::PlayerObservation& current,
                                             const CertifiedEnvironmentConfig& config,
                                             const EpisodeSpec& spec) {
    if (!safe_key_shape(request, candidate)) {
        throw BoundaryError(FailureCode::PrivacyInvariant, FailureStage::Projection,
                            "semantic action key is not proven safe for the public perspective");
    }
    const auto visible_source = candidate.source_card == 0
                                    ? nullptr
                                    : visible_card_for_reference(current, candidate.source_card,
                                                                 candidate.source_controller,
                                                                 candidate.source_location,
                                                                 candidate.source_sequence);
    const bool known_source = candidate.source_card != 0 &&
                              known_own_deck_identity(config, spec, current, candidate.source_card,
                                                      candidate.source_controller, candidate.source_location);
    if (candidate.source_card != 0) {
        if (!known_source &&
            (!observation::candidate_observation_consistent(current, candidate) ||
             visible_source == nullptr)) {
            throw BoundaryError(FailureCode::PrivacyInvariant, FailureStage::Projection,
                                "source card identity is not visible to the acting perspective");
        }
    }
    const auto visible_target = candidate.target_card == 0
                                    ? nullptr
                                    : visible_card_for_reference(current, candidate.target_card,
                                                                 candidate.target_controller,
                                                                 candidate.target_location,
                                                                 candidate.target_sequence);
    const bool known_target = candidate.target_card != 0 &&
                              known_own_deck_identity(config, spec, current, candidate.target_card,
                                                      candidate.target_controller, candidate.target_location);
    if (candidate.target_card != 0) {
        if (!known_target &&
            (!observation::candidate_observation_consistent(current, candidate) ||
             visible_target == nullptr)) {
            throw BoundaryError(FailureCode::PrivacyInvariant, FailureStage::Projection,
                                "target card identity is not visible to the acting perspective");
        }
    }

    EnvironmentActionCandidate result;
    result.action_kind = public_action_kind(candidate.action_kind);
    result.semantic_key = candidate.semantic_key;
    if (candidate.source_card != 0) {
        result.source_card = candidate.source_card;
        result.source_controller = candidate.source_controller;
        result.source_location = candidate.source_location;
        if (visible_source != nullptr) {
            result.source_sequence = candidate.source_sequence;
        }
        if (candidate.source_position != 0 && visible_source != nullptr) {
            result.source_position = candidate.source_position;
        }
    } else if (candidate.source_location != 0) {
        result.source_controller = candidate.source_controller;
        result.source_location = candidate.source_location;
        result.source_sequence = candidate.source_sequence;
    }
    if (candidate.target_card != 0) {
        result.target_card = candidate.target_card;
        result.target_controller = candidate.target_controller;
        result.target_location = candidate.target_location;
        if (visible_target != nullptr) {
            result.target_sequence = candidate.target_sequence;
        }
    } else if (candidate.target_location != 0) {
        result.target_controller = candidate.target_controller;
        result.target_location = candidate.target_location;
        result.target_sequence = candidate.target_sequence;
    }
    ...
    result.source_index = candidate.source_index;
    result.amount = candidate.amount;
    result.continuation_id = candidate.continuation_id;
    result.submits_engine_response = candidate.submits_engine_response;
    return result;
}
...
EnvironmentContinuationView project_continuation(
    ...
    result.continuation_id = continuation.continuation_id;
    ...
    result.original_message_type = continuation.original_message_type;
    ...
    result.peak_candidate_count = continuation.peak_candidate_count;
    result.terminal_solution_count = continuation.terminal_solution_count;
...
struct EpisodicEnvironment::Impl final {
    ...
    std::array<std::optional<observation::PlayerObservation>, 2> terminal_views;
    ...
    StepRejected rejected(const ActionSelection& selection, const RejectionCode code) const {
        ...
        result.submitted_semantic_decision_id = selection.semantic_decision_id;
        result.submitted_submission_token = selection.submission_token;
        result.submitted_semantic_key = selection.semantic_key;
        result.current_episode_semantic_id = current_episode_id;
        if (current_frame.has_value()) {
            result.current_semantic_decision_id = current_frame->semantic_decision_id;
            result.current_candidate_domain_digest = current_frame->candidate_domain_digest;
        }
        return result;
    }
    ...
    EpisodeFailure make_failure(const FailureCode code, const FailureStage stage,
        ...
        if (driver != nullptr) {
            ...
            if (current_frame.has_value()) {
                result.last_semantic_decision_id = current_frame->semantic_decision_id;
    ...
    DecisionFrame project_frame(const DriverDecisionBoundary& boundary,
        ...
        if (current.schema_version != kObservationContractId || current.perspective_player != request.player ||
            current.engine_step_index != request.engine_step_index || current.decision_index != decision_index ||
        ...
        public_request.decision_id = request.decision_id;
        public_request.engine_step_index = request.engine_step_index;
        public_request.player = request.player;
        public_request.engine_message_type = request.engine_message_type;
        public_request.engine_message_name = request.engine_message_name;
        if (request.continuation.has_value()) {
        ...
        for (const auto& candidate : request.candidates) {
            auto projected = project_candidate(request, candidate, current, config, current_spec);
            if (request.continuation.has_value() &&
                projected.continuation_id != public_request.continuation->continuation_id) {
                throw BoundaryError(FailureCode::PublicFrameInvariant, FailureStage::Projection,
                                    "candidate continuation linkage is not complete");
            }
            keys.push_back(candidate.semantic_key);
            public_request.candidates.push_back(std::move(projected));
        }
        ...
        result.acting_player = request.player;
        result.observation = current;
        result.request = std::move(public_request);
        result.candidate_domain_digest = candidate_domain_digest(protocol::decision_kind_name(request.kind), keys);
        result.semantic_decision_id = semantic_decision_id(
            result.episode_semantic_id, result.decision_index, request.decision_id, result.acting_player,
            result.engine_step_index, result.observation.observation_hash, result.candidate_domain_digest);
        if (result.acting_player != result.request.player ||
            result.acting_player != result.observation.perspective_player ||
            result.request.candidates.size() != keys.size()) {
            throw BoundaryError(FailureCode::PublicFrameInvariant, FailureStage::Projection,
        ...
        result.submission_token = {episode_incarnation_counter, frame_generation_counter};
        if (!result.submission_token.valid()) {
            throw BoundaryError(FailureCode::TokenNamespaceExhausted, FailureStage::Projection,
    ...
    NextBoundary consume_boundary(const DriverBoundary& boundary, const bool accepted_action) {
        if (const auto* decision = std::get_if<DriverDecisionBoundary>(&boundary)) {
            ...
            auto frame = project_frame(*decision, next_index);
        ...
        if (const auto* reached_terminal = std::get_if<DriverGameTerminal>(&boundary)) {
            ...
            terminal_views[0] = *reached_terminal->player_zero_observation;
            terminal_views[1] = *reached_terminal->player_one_observation;
            driver.reset();
            current_frame.reset();
            terminal = value;
            interruption.reset();
    ...
    StepResult step(const ActionSelection& selection) {
        ...
        if (selection.semantic_decision_id != current_frame->semantic_decision_id) {
            return rejected(selection, RejectionCode::WrongSemanticDecision);
        }
        const auto selected_it = std::find_if(
            ...
            [&selection](const EnvironmentActionCandidate& candidate) {
                return candidate.semantic_key == selection.semantic_key;
            });
        if (selected_it == current_frame->request.candidates.end()) {
            return rejected(selection, RejectionCode::UnknownSemanticKey);
        }

        const auto accepted_episode_id = current_frame->episode_semantic_id;
        const auto accepted_decision_id = current_frame->semantic_decision_id;
        ...
        transition.semantic_decision_id = accepted_decision_id;
        transition.decision_index = accepted_decision_index;
        transition.selected_semantic_key = selection.semantic_key;

        DriverApplyResult apply_result;
        try {
            apply_result = driver->apply_semantic_key(selection.semantic_key);
        ...
        if (!apply_result.accepted.has_value() ||
            apply_result.accepted->selected_semantic_key != selection.semantic_key ||
...
std::optional<observation::PlayerObservation> EpisodicEnvironment::perspective_terminal_view(
