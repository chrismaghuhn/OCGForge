...
bool validate_id_vector(const std::vector<std::string>& values,
    ...
    for (std::size_t index = 0; index < values.size(); ++index) {
        if (!detail::canonical_token(values[index])) {
            set_diagnostic(diagnostic, std::string("V3 ") + name + " is not canonical");
            return false;
        }
        if (index > 0 && !(values[index - 1] < values[index])) {
            set_diagnostic(diagnostic, std::string("V3 ") + name + " is not sorted");
            return false;
        }
    }
    return true;
...
bool validate_candidate_evaluation_v3(const CandidateEvaluation& value,
                                      std::string* diagnostic) {
    if (!environment::is_public_action_key_v3(value.public_action_key)) {
        set_diagnostic(diagnostic, "V3 candidate evaluation has an invalid public action key");
        return false;
    }
    if (!valid_evaluation_status(static_cast<std::uint8_t>(value.status))) {
        set_diagnostic(diagnostic, "V3 candidate evaluation status is unknown");
        return false;
    }
...
bool validate_teacher_ranking_result_v3(
    ...
    try {
        if (!valid_ranking_status(static_cast<std::uint8_t>(value.status))) {
            set_diagnostic(diagnostic, "V3 teacher ranking status is unknown");
            return false;
        }
        if (value.fallback_level.has_value() &&
            !valid_fallback_level(static_cast<std::uint8_t>(*value.fallback_level))) {
            set_diagnostic(diagnostic, "V3 teacher fallback level is unknown");
            return false;
        }
        if (value.proposed_state_delta.has_value() &&
            !validate_teacher_state_delta_v3(*value.proposed_state_delta)) {
            set_diagnostic(diagnostic, "V3 teacher state delta is invalid");
            return false;
        }
        for (std::size_t index = 0; index < value.evaluations.size(); ++index) {
            if (!validate_candidate_evaluation_v3(value.evaluations[index], diagnostic)) {
                return false;
            }
            for (std::size_t previous = 0; previous < index; ++previous) {
                if (value.evaluations[previous].public_action_key ==
                    ...
                    set_diagnostic(diagnostic,
                                   "V3 evaluation contains duplicate public action keys");
                    return false;
                }
            }
        }
        ...
        if (value.status != TeacherRankingStatus::Selected) {
            if (value.selected_public_action_key.has_value() ||
                ...
                set_diagnostic(diagnostic,
                               "non-selected V3 result carries an actionable result");
                return false;
            }
            return true;
        }
        ...
        if (!value.selected_public_action_key.has_value() ||
            !environment::is_public_action_key_v3(*value.selected_public_action_key)) {
            set_diagnostic(diagnostic,
                           "selected V3 result lacks a valid public action key");
            return false;
        }
        if (value.proposed_state_delta.has_value() &&
            ...
            set_diagnostic(diagnostic,
                           "V3 state delta does not match selected public action key");
            return false;
        }
        ...
        if (selected == nullptr) {
            set_diagnostic(diagnostic,
                           "selected V3 action is absent from evaluation records");
            return false;
        }
        if (selected->status != CandidateEvaluationStatus::Supported) {
            set_diagnostic(diagnostic,
                           "selected V3 action is not Supported");
            return false;
        }
        if (!selected->score.has_value()) {
            set_diagnostic(diagnostic, "selected V3 action has no score");
            return false;
        }
        if (!value.selected_score_vector.has_value()) {
            set_diagnostic(diagnostic, "selected V3 result has no selected score");
            return false;
        }
        if (*selected->score != *value.selected_score_vector) {
            set_diagnostic(diagnostic, "selected V3 score does not match its evaluation");
            return false;
        }
        if (!value.fallback_level.has_value()) {
            set_diagnostic(diagnostic, "selected V3 result has no fallback level");
            return false;
        }
        return true;
        ...
        set_diagnostic(diagnostic, "V3 teacher ranking validation threw");
        return false;
    }
}
...
policy::PolicySelection teacher_policy_selection_from_result_v3(
    ...
    std::string diagnostic;
    if (!validate_teacher_ranking_result_v3(value, &diagnostic)) {
        return error_selection(policy::PolicyErrorCode::InvalidCandidateDomain,
                               diagnostic.empty() ? "invalid V3 teacher ranking result"
                                                   : diagnostic);
    }
    switch (value.status) {
    ...
    case TeacherRankingStatus::InvalidInput:
        return error_selection(policy::PolicyErrorCode::InvalidCandidateDomain,
                               "V3 teacher rejected its public input");
    case TeacherRankingStatus::Blocked:
        return error_selection(policy::PolicyErrorCode::InvalidConfiguration,
                               "V3 teacher decision is blocked");
    case TeacherRankingStatus::Unsupported:
        return error_selection(policy::PolicyErrorCode::InvalidConfiguration,
                               "V3 teacher decision is unsupported");
    }
    return error_selection(policy::PolicyErrorCode::InvalidConfiguration,
                           "V3 teacher ranking status is unknown");
}
