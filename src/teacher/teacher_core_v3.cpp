...
void append_diagnostic_evaluation(
    ...
    TeacherCandidateEvaluationDiagnosticsV3 value;
...
std::optional<EpisodeLocalStrategyStateV3> apply_public_completion_v3(
    const StrategyProfileV1& profile,
    const EpisodeLocalStrategyStateV3& reconciled_state,
    ...
    if (result.active_line_id.has_value()) {
        ...
        const auto line_selection = select_goal_and_line_v3(profile, result, facts);
        ...
        if (line_selection.status == PredicateEvaluationStatus::True &&
            ...
            for (const auto& node_id : line_selection.ready_node_ids) {
                ...
                if (evaluate_node_completion_v3(*node, accepted, observation, owner, profile) ==
    ...
    if (evaluate_goal_completion_v3(*goal, accepted, observation, owner, profile) ==
        PredicateEvaluationStatus::True) {
...
TeacherRankingResultV3 TeacherCoreV3::propose(
    const ygo::policy::PolicyInput& input,
    const StrategyProfileV1& profile,
    const EpisodeLocalStrategyStateV3& state,
    TeacherRankingDiagnosticsV3* diagnostics) const {
    try {
        if (diagnostics != nullptr) *diagnostics = {};
        const auto invalid = [&](const char*) {
            return invalid_result_v3(TeacherRankingStatus::InvalidInput, input.candidates);
        };
        if (!validate_strategy_profile(profile) ||
            !validate_strategy_state_v3(state) ||
        ...
        const auto domain_valid = evaluate_candidate_domain_v3(
            input.candidates,
            [&](const environment::EnvironmentActionCandidate& candidate) {
                CandidateFeatures features;
                if (!extract_candidate_features_v3(candidate, facts_result.snapshot, features)) {
                    if (f0_applicable) {
                        append_stage_value(
                ...
                if (f0_applicable) {
                    active_outcome = evaluate_goal_line_progress_v3(
                ...
                if (f1_applicable) {
                    ...
                    f1_outcome = evaluate_goal_line_progress_v3(
        ...
        const auto result_before_delta = resolve_teacher_fallback_v3(input.candidates, stages);
        if (diagnostics != nullptr) {
        ...
        const auto delta = propose_teacher_state_delta_v3(
        ...
        if (!validate_teacher_ranking_result_v3(result, &diagnostic)) {
