...
PolicySelection TeacherRunnerV4::failure(const PolicyErrorCode code,
...
TeacherRunnerV4CreateResult TeacherRunnerV4::create(
    TeacherRunnerV4Config config) noexcept {
    try {
        ...
        for (std::uint8_t player = 0; player < 2; ++player) {
            if (!config.sessions[player].has_value()) {
                return {std::nullopt,
                        PolicyError{PolicyErrorCode::InvalidConfiguration,
                                    "V4 Teacher runner lacks a session for a player"}};
            }
            ...
            if (session.assignment.player != player ||
                ...
                return {std::nullopt,
                        PolicyError{PolicyErrorCode::InvalidConfiguration,
                                    diagnostic.empty()
                                        ? "V4 Teacher session does not match its assignment"
                                        : diagnostic}};
            }
            ...
            if (deck_role > 1 || deck_role_seen[deck_role] ||
                ...
                return {std::nullopt,
                        PolicyError{PolicyErrorCode::InvalidConfiguration,
                                    "V4 Teacher runner requires two distinct deck roles, assignments, and artifacts"}};
            }
        ...
        if (!deck_role_seen[0] || !deck_role_seen[1]) {
            return {std::nullopt,
                    PolicyError{PolicyErrorCode::InvalidConfiguration,
                                "V4 Teacher runner requires one session per locked deck role"}};
        }
        return {std::optional<TeacherRunnerV4>(
                    TeacherRunnerV4(std::move(config))),
                std::nullopt};
        ...
        return {std::nullopt,
                PolicyError{PolicyErrorCode::InvalidConfiguration,
                            "V4 Teacher runner construction failed"}};
    }
}

PolicySelection TeacherRunnerV4::select(const environment::DecisionFrame& frame) noexcept {
    return select_impl(frame, nullptr);
}

PolicySelection TeacherRunnerV4::select_with_diagnostics(
...
PolicySelection TeacherRunnerV4::select_impl(
    const environment::DecisionFrame& frame,
    teacher::TeacherRankingDiagnosticsV3* diagnostics) noexcept {
    if (pending_player_.has_value()) {
        return failure(PolicyErrorCode::LifecycleFailure,
                       "V4 Teacher runner has an unresolved pending proposal");
    }
    if (frame.contract_id != environment::kEpisodicEnvironmentV4ContractId ||
        ...
        return failure(PolicyErrorCode::InvalidConfiguration,
                       "V4 Teacher runner received an invalid V4 public frame");
    }
...
TeacherRunnerV4ActionResult TeacherRunnerV4::select_action(
    const environment::DecisionFrame& frame) noexcept {
    if (!frame.submission_token.valid()) {
        return {std::nullopt,
                PolicyError{PolicyErrorCode::InvalidConfiguration,
                            "V4 Teacher runner received an invalid submission token"}};
    }
...
bool TeacherRunnerV4::commit(const environment::StepAccepted& accepted) noexcept {
    if (!pending_player_.has_value()) {
        return false;
    }
...
bool TeacherRunnerV4::reject_pending_proposal() noexcept {
    if (!pending_player_.has_value()) {
        return false;
    }
